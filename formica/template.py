...
class Template():
    ...
    def module(self, module_name, part='*', **variables):
        ...
        self.load(filepath.rstrip('/') + '/' + module_name.strip('/'), part, variables)

    def load(self, path=os.getcwd(), part='*', variables={}):
        formica_commands = {
            'resource': lambda resource: self.cftemplate.add_resource(resource),
            'mapping': lambda name, values: self.cftemplate.add_mapping(name, values),
            'description':
                lambda description: self.cftemplate.add_description(description),
            'metadata': lambda metadata: self.cftemplate.add_metadata(metadata),
            'condition':
                lambda name, condition: self.cftemplate.add_condition(name, condition),
            'module': self.module,
            'name': helper.name}
